...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sit_in_submit'])) {
    ...
    $check_sql = "SELECT * FROM sitin_records WHERE idnumber = '$id_number' AND TIME_OUT IS NULL";
    ...
    if ($check_result->num_rows > 0) {
        ...
        $sql_sitin = "INSERT INTO sitin_records (ID, idnumber, PURPOSE, LABORATORY, TIME_IN) 
                     VALUES ('$next_id', '$idnumber', '$purpose', '$laboratory', '$checkin_time')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sign_out'])) {
    ...
    try {
        ...
        $update_sql = "UPDATE sitin_records SET TIME_OUT = '$checkout_time' 
                      WHERE idnumber = '$idnumber' AND TIME_OUT IS NULL";
        ...
        $update_session = "UPDATE user SET session_count = session_count - 1 WHERE idnumber = '$idnumber'";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    if (strlen($search_term) < 2) {
        ...
        if (is_numeric($search_term)) {
            $sql_search = "SELECT * FROM user WHERE idnumber = '$search_term'";
        } else {
            // Search by name
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('idnumber').value = studentData.id;
